  ...
  <div className="mb-8">
    <div className="mb-4 flex items-center gap-2">
      ...
      <span className="text-xs font-semibold uppercase tracking-wide text-[#00D4AA]">
        Live AI Models ·{" "}
        {(data.aiSearchVisibility.modelsCalled || []).join(" · ") ||
    ...
    <div className="mb-6 grid gap-4 md:grid-cols-4">
      <MetricCard
        ...
        value={`${data.aiSearchVisibility.overallScore}/100`}
        score={Number(data.aiSearchVisibility.overallScore || 0)}
        tooltip="Live score across ChatGPT, Claude and Gemini responses."
      />
      <MetricCard
        label="Visibility Rate"
        value={`${data.aiSearchVisibility.visibilityRate}%`}
        score={Number(data.aiSearchVisibility.visibilityRate || 0)}
        tooltip="Share of tested prompts where your brand appeared."
      />
      <MetricCard
        label="Avg Position"
        value={
          data.aiSearchVisibility.avgPosition
            ? `${data.aiSearchVisibility.avgPosition} / 5`
            : "—"
        }
        tooltip="Average rank of your brand when mentioned (1 = first)."
      />
      <MetricCard
        label="Sentiment Score"
        value={`${data.aiSearchVisibility.sentimentScore}/100`}
        score={Number(data.aiSearchVisibility.sentimentScore || 0)}
        tooltip="How positive the mentions of your brand are."
      />
    </div>

    {/* per-model visibility row */}
    <div className="mb-6 grid gap-4 sm:grid-cols-3">
      {[
        ["ChatGPT", data.aiSearchVisibility.modelBreakdown?.chatgpt],
        ["Claude", data.aiSearchVisibility.modelBreakdown?.claude],
        ["Gemini", data.aiSearchVisibility.modelBreakdown?.gemini],
      ].map(([name, pct]: any) => (
        <div
          key={name}
          className="rounded-2xl border border-[#1e3a5f] bg-[#0E2440] p-4"
        >
          <p className="text-xs font-semibold uppercase tracking-wide text-[#64748B]">
            {name}
          </p>
          <p className="mt-1 text-2xl font-extrabold text-white">{pct ?? 0}%</p>
          <div className="mt-2 h-1.5 w-full rounded-full bg-[#0B1929]">
            <div
    ...
    {data.aiSearchVisibility.promptResults?.length > 0 && (
      <div className="mb-6 overflow-hidden rounded-2xl border border-[#1e3a5f] bg-[#0E2440]">
        <div className="border-b border-[#1e3a5f] px-5 py-3">
          <h3 className="font-semibold text-white">Prompt Results</h3>
          <p className="text-xs text-[#64748B]">
            How each AI model answered — ✅ mentioned, ❌ missing.
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-[#64748B]">
                ...
                <th className="px-3 py-3 text-center">ChatGPT</th>
                <th className="px-3 py-3 text-center">Claude</th>
                <th className="px-3 py-3 text-center">Gemini</th>
                <th className="px-3 py-3 text-center">Avg Pos</th>
              </tr>
            </thead>
            <tbody>
              {data.aiSearchVisibility.promptResults.map((row: any, i: number) => {
                ...
                return (
                  <tr key={i} className="border-t border-[#13294a]">
                    ...
                    <td className="px-3 py-3 text-center text-[#94A3B8]">
                      {row.avgPosition ?? "—"}
                    </td>
                  </tr>
                );
    ...
    <div className="mb-8 grid gap-4 lg:grid-cols-2">
        ...
        {data.aiSearchVisibility.topCompetitors?.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {data.aiSearchVisibility.topCompetitors.map((c: string, i: number) => (
              <span
                key={i}
                className="rounded-full border border-[#1e3a5f] bg-[#122B4E] px-3 py-1 text-xs text-[#E2E8F0]"
              >
                {c}
              </span>
            ))}
          ...
          <p className="text-sm text-[#64748B]">
            No competitors detected across responses.
          </p>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-[#00D4AA]/30 bg-[#00D4AA]/5 p-5">
        ...
        <p className="mb-3 text-xs text-[#94A3B8]">
          Prompts where your brand was not mentioned — turn these into content.
        </p>
        {data.aiSearchVisibility.missedPrompts?.length > 0 ? (
          ...
          <p className="text-sm text-[#00D4AA]">
            Your brand appeared in every tested prompt. 🎉
          </p>
        )}
      </div>
    </div>
